...
def register_class(db: Session, class_id: int, user: User):
    ...
    # Tạm thời bỏ kiểm tra trùng lịch để test
    # if check_schedule_conflict(db, user.id, class_obj.schedule, exclude_class_id=class_id):
    ...
    try:
        ...
        if not reg:
            raise HTTPException(status_code=400, detail="Không thể tạo đăng ký.")
